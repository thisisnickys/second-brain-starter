...
function buildPlanPrompt(currentPlan, message, now, busy) {
  ...
  return [
    ...
    '"""',
    '',
    'Return the FULL updated plan. If a plan already exists, treat her message as an',
    'EDIT: apply her corrections, add or remove the items she mentions, and KEEP every',
    'other item unchanged. Do not drop things she did not ask to change.',
    '',
    ...
    '  If she corrects a date ("no, make it today"), update the date on those items.',
    ...
    ...(busyList.length ? [
      ...
      '  KEEP her stated time — never silently move it — and set "overlaps" on that item to',
      '  the conflicting block\'s title, e.g. { ..., "overlaps": "Deep work" }, so she can',
      '  adjust. Items with no conflict omit "overlaps" or set it to null.',
    ] : []),
    'If she clearly says start over / scrap it, return only the new items (or [] if none).',
  ].join('\n');
}
